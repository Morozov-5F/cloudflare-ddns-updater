...
parser = argparse.ArgumentParser(
    description="Update DNS record in Cloudflare DNS service")
...
parser.add_argument('-t', '--token', required=True,
                    help="Access token for modifying the zone")
...
headers = {'X-Auth-Email': auth_email,
           'Authorization': f'Bearer {auth_key}', 'Content-Type': 'application/json'}
...
if zone_name and not zone_id:
    ...
    if not zone_resp.ok or not resp['success']:
        print(
            f'error: Unable to get the zone info, status {zone_resp.status_code}, response: "{zone_resp.text}"')
        exit(1)
...
dns_record_resp = requests.get(
    f'https://api.cloudflare.com/client/v4/zones/{zone_id}/dns_records?name={record_name}', headers=headers)
dns_record = dns_record_resp.json()
if not dns_record_resp.ok or not dns_record['success']:
    print(
        f'error: Unable to get the DNS record info, status {dns_record_resp.status_code}, response: "{dns_record_resp.text}"')
    exit(1)
...
if not current_ip_resp.ok:
    print(
        f'error: unable to get the current IP, server responded with "{current_ip_resp.status_code}" code, response text: {current_ip_resp.text}')
    exit(1)
...
if current_ip == old_ip:
    print(
        f'Old IP ({old_ip}) is equal to Current IP ({current_ip}), skipping update')
    exit(0)
...
update_resp = requests.put(
    f'https://api.cloudflare.com/client/v4/zones/{zone_id}/dns_records/{record_id}', headers=headers, json=data)
if not update_resp.ok or not update_resp.json()['success']:
    print(
        f'error: Unable to update the DNS record info, status {update_resp.status_code}, response: "{update_resp.text}"')
    exit(1)
